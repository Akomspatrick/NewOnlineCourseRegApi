...
namespace NewOnlineCourseReg.Domain.CourseRegistration
{
    public partial  class Student : IAggregateRoot
    {
        ...
        {
           var courselist= this.CourseRegistrationForms.Where(p => p.semester == course.semester && p.session == course.session).ToList();
            if (courselist != null)
            {
        ...
        {
            var courselist = this.CourseRegistrationForms.Where(p => p.semester == course.semester && p.session == course.session).FirstOrDefault();
            if (courselist != null)
            {
                return courselist;
            }
            ...
            {
                return CourseRegistrationForm.InitialiseCourseRegistrationForm(this.StudentId, course.session, course.semester, new List<Course>(), 0, 0);
            }
        }
